...
export function getCatalogMenuList(src: any[]): Menu.CatalogMenuOptions[] {
  let list: Menu.CatalogMenuOptions[] = [];
  if (src && src.length > 0) {
    src.forEach(x => {
      list.push({
        id: x.id,
...
export function packageAuthMenuList(src: any[], id: string): Menu.MenuOptions[] {
  if (!src || !id) {
    return [];
  }
...
function packageMenuItem(item: any) {
  ...
  if (!item.url) return menu;

  if (item.url.startsWith("/")) {
    if (item.url.indexOf("/record/templateslist.html") > 0) {
      // 模板页面
      menu.component = "/template/index";
      menu.path = "/template/index";
    } else if (item.url.indexOf("/custom/") > 0) {
      // 自定义页面
    ...
    if (item.open_type === "1") {
      // 内嵌页面
      menu.component = "/embedded/index";
      menu.path = "/embedded/index";
    } else if (item.open_type === "2") {
      // 外链页面
      menu.component = "";
      menu.path = "";
      menu.meta.isLink = item.url;
    } else {
      // 不做处理
    }
  }
